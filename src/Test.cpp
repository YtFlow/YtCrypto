...
namespace YtCrypto
{
	...
	unsigned int Test::Encrypt(const Platform::Array<uint8, 1u>^ encData, Platform::WriteOnlyArray<uint8, 1u>^ outData)
	{
		size_t len;
		mbedtls_cipher_update(encctx, encData->begin(), encData->Length, outData->begin(), &len);
		return len;
	}
	unsigned int Test::Decrypt(const Platform::Array<uint8, 1u>^ decData, Platform::WriteOnlyArray<uint8, 1u>^ outData)
	{
		auto realDecData = decData->begin();
		auto realLen = decData->Length;
		if (!dec_iv_inited) {
